# QOwnNotes - GitHub Copilot Instructions
...
2. **The project uses Qt/C++** - Be mindful of Qt's signal/slot mechanism, QObject lifecycle, and Qt-specific patterns
3. **Submodules are used** - Libraries in `src/libraries/` are git submodules
4. **Multiple Qt versions supported** - Code must work with both Qt5 and Qt6
5. **Cross-platform** - Code must work on Linux, macOS, and Windows
6. **Two build systems available** - Primary is qmake (`.pro` files), but CMake (`CMakeLists.txt`) is also supported
7. **Development mode** - Use `CONFIG+=DEV_MODE` for qmake or `-DDEV_MODE=ON` for CMake in active development
