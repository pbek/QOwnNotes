...
void Utils::Cli::generateFishCompletionScript(const QList<QCommandLineOption>& options,
    ...
    //    // Introduction of the function in Fish script - replace {appName} with your actual app
    //    name std::cout << "function __fish_" << appName.toStdString() << "_complete" << std::endl;
    //    std::cout << "    complete -c " << appName.toStdString() << " -f -n
    ...
    //        }
    //
    //        std::cout << " -a \"" << fishOption.toStdString() << "\" -d \"" <<
    ...
    //    std::cout << "complete -c " << appName.toStdString() << " -f -a \"(__fish_" <<
    //    appName.toStdString() << "_complete)\"" << std::endl;
    //
    //    std::cout << "# Fish shell completion script for " << appName.toStdString() << std::endl;
    ...
    //            std::string optionName = name.length() > 1 ? "--" + name.toStdString() : "-" +
    //            name.toStdString(); std::cout << "        case '" << optionName << "'" <<
    //            std::endl;
    //        }
    //    }
    //
    ...
    //    std::cout << "    complete -c " << appName.toStdString() << " -f -n
    //    '__fish_seen_subcommand_from ;' -a '"; for (const QCommandLineOption& option : options) {
    //        for (const QString& name : option.names()) {
    //            std::string optionName = name.length() > 1 ? "--" + name.toStdString() : "-" +
    //            name.toStdString(); std::cout << optionName << " ";
    //        }
    //    }
    //    std::cout << "'" << std::endl;
    //
    ...
    for (const QCommandLineOption& option : options) {
        ...
        for (const QString& longName : option.names().filter(QRegExp("^[^-].*"))) {
            std::cout << "complete -c " << appName.toStdString() << " -l "
        ...
        for (const QString& shortName : option.names().filter(QRegExp("^-.$"))) {
            std::cout << "complete -c " << appName.toStdString() << " -s "
                      << shortName.toStdString().substr(1);    // Remove the '-' prefix
            if (!option.description().isEmpty()) {
                std::cout << " -d \"" << option.description().toStdString() << "\"";
            }
            std::cout << std::endl;
