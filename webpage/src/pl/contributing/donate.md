# Wspomóż projekt

QOwnNotes nie generuje żadnych przychodów (projekt typu non-profit).

Jeśli podoba Ci się oprogramowanie QOwnNotes i chcesz **wspomóc** projekt finansowo, aby pomóc pokryć koszty hostingu, domeny itp., możesz to zrobić w sposób opisany poniżej.

## PayPal

Możesz **przekazać donację za pośrednictwem PayPal**, klikając poniższy przycisk:

<form id="paypal-donate" action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_blank">
    <input type="hidden" name="cmd" value="_s-xclick">
    <input type="hidden" name="encrypted" value="-----BEGIN PKCS7----------END PKCS7-----
">
    <input type="image" src="https://www.paypalobjects.com/en_US/i/btn/btn_donateCC_LG.gif" name="submit" alt="PayPal - The safer, easier way to pay online!">
    <img alt="" src="https://www.paypalobjects.com/de_DE/i/scr/pixel.gif" width="1" height="1">
</form>

## Liberapay

Możesz **przekazać donację za pośrednictwem Liberapay**, klikając poniższy przycisk:

<a href="https://liberapay.com/pbek/donate"><img alt="Przekaż donację za pośrednictwem Liberapay" src="https://liberapay.com/assets/widgets/donate.svg"></a></noscript>

## Bitcoin

Możesz **przekazać donację w kryptowalucie Bitcoin (BTC)** na następujący adres Bitcoin BTC:

```
395NiU7QKpNiayWgRrUEGZDn8Z1ebMx8CH
```

Możesz **przekazać donację w kryptowalucie Bitcoin Cash** na następujący adres Bitcoin Cash:

```
qpk3m6putywwad9sycz9hny5azs7dz67x5jrqmcelq
```

## Ethereum

Możesz **przekazać donację w kryptowalucie Ethereum (ETH)** na następujący adres Ethereum ETH:

```
0x992aD2066640C5bF175c7c5649a41827dF6f87C9
```

Możesz **przekazać donację w kryptowalucie Ethereum (ETH)** na następujący adres Ethereum ETH:

```
0xcC809a58637372EaC340E53767649cdc5C53cA0D
```
