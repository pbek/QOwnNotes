...
| `--decrypt-note <file>`              | 암호화된 노트 파일의 복호화된 텍스트를 명령줄에 인쇄하고 종료합니다                                 |
| `--decrypt-note-password <password>` | 비밀번호를 요청하는 대신 `--decrypt-note`의 비밀번호를 제공합니다                           |

::: tip
QOnNotes 설치 시 문제가 발생할 경우 `--session` 매개 변수를 사용하여 현재 설정을 잃지 않고 새로운 설정으로 응용프로그램을 시작할 수 있습니다.
...
::: tip
설정 덤프를 클립보드에 직접 복사하여 [GitHub 문제](https://github.com/pbek/QOwnNotes/issues)에 게시하려면 Linux의 경우 `xclip` 또는 `xsel`, macOS의 경우 `pbcopy`, Windows의 경우 `clip`을 사용할 수 있습니다:
...
매개변수 `--decrypt-note <file>`을 사용하면 암호화된 노트 파일의 복호화된 텍스트를 명령줄에 인쇄할 수 있습니다. 비밀번호가 제공되지 않으면, QOwnNotes가 명령줄에서 비밀번호를 요청할 것입니다.

```bash
QOwnNotes --decrypt-note secret.md
```

비밀번호를 `--decrypt-note-password`으로 직접 제공할 수도 있습니다:

```bash
QOwnNotes -- decrypt 노트 secret.md -- decrypt 노트 비밀번호 "내 비밀번호"
```

::: warning

비밀번호를 명령줄 매개변수로 제공하면 셸 기록이나 프로세스 목록에 비밀번호가 노출될 수 있습니다. 이 문제가 있는 경우 비밀번호 프롬프트를 선호합니다.
:::
...
::: tip
메뉴 동작의 객체 이름은 [mainwindow.ui](https://github.com/pbek/QOwnNotes/blob/main/src/mainwindow.ui)에서 얻을 수 있습니다. 영어 메뉴 제목만 검색해 보세요. 이러한 텍스트는 시간이 지남에 따라 변경될 수 있습니다.
:::
